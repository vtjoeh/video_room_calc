 ...
 * file). These const aliases bring the names back into the local
 * lexical scope so the call sites scattered through this file stay
 * unchanged. They sit at the very top because `createUuid()` is called
 * during top-level init (sessionId, roomObj.roomId) and `const` bindings
 * are not hoisted; they must precede their first use.
 *
 * `window.convertMetersFeet` is also exposed because RoomCalculator.html
 * still has an inline `onChange="convertMetersFeet()"` handler on the
 * meters/feet dropdown — inline handlers run in the global object's
 * scope and can't see lexical const declarations. */
...
function onLoad() {
    ...
    if (localStorage.getItem('autoZposition') === 'true') {
        ...
        document.getElementById('autoZposition').checked = false;
...
function showEntireFloor() {
    ...
    if (activeRoomNode) {
        ...
        activeRoomNode.fill('#ADD8E655');
        activeRoomNode.listening(true);
...
function drawRoom(redrawShapes = false, dontCloseDetailsTab = false, dontSaveUndo = false, isZoomingRoomPart = false) {
    ...
    if (redrawShapes) {
        ...
        roomObjToCanvas(isZoomingRoomPart);
...
let selectionRectangle = new Konva.Rect({
    fill: 'rgba(0, 110, 255, 0.3)',
    strokeWidth: 1,
    stroke: 'rgba(0, 110, 255)',
    visible: false,
    listening: false,
});
...
function roomObjToCanvas(updateExisting = false) {
...
function insertTable(insertDevice, groupName, attrs, uuid, selectTrNode) {
    ...
    } else if (insertDevice.id === 'tblRect') {
        tblWallFlr = new Konva.Rect({
            x: pixelX,
            y: pixelY,
            rotation: rotation,
    ...
    } else if (insertDevice.id === 'tblUnknownObj') {
        tblWallFlr = new Konva.Rect({
            x: pixelX,
            y: pixelY,
            rotation: rotation,
    ...
    } else if (insertDevice.id === 'credenza') {
        tblWallFlr = new Konva.Rect({
            x: pixelX,
            y: pixelY,
            rotation: rotation,
    ...
    } else if (insertDevice.id === 'tblTrap') {
        tblWallFlr = new Konva.Shape({
            x: pixelX,
            y: pixelY,
            rotation: rotation,
    ...
    } else if (insertDevice.id === 'tblCurved') {
        tblWallFlr = new Konva.Shape({
            x: pixelX,
            y: pixelY,
            rotation: rotation,
    ...
    canvasNodesMap.set(tblWallFlr.id(), tblWallFlr);
}

/* todo: fix slipping of when stretching walls or tables */
function updateTransformedWall(node) {
    return;
...
function changeZheightOfItem(dragItem, dragNode) {
    ...
    if (dragItem.data_deviceid.startsWith('ptz')) {
        compareToGroups = [groupVideoDevices, groupDisplays];
    } else {
            ...
            if (!(node.data_deviceid === 'boxRoomPart' || node.data_deviceid === 'polyRoom' || node.id() === dragNode.id()) && 'data_deviceid' in node) {
                ...
                if (doPolygonsIntersect2(fourCorners, dragNodeFourCorners)) {
                    ...
                    if (newPoints.length > 0) {
                        ...
                        if (dragNode.data_deviceid.startsWith('webcam')) {
                            height = height + 0.08 / ((roomObj.unit === 'feet') ? 1 : 3.28084);
                        }
                        ...
                        document.getElementById('itemZposition').style.backgroundColor = '#ffcc5449';
...
function insertShapeItem(deviceId, groupName, attrs, uuid = '', selectTrNode = false) {
    ...
    allNodeShapeGroups.forEach(shapeGroup => {
        if (shapeGroup.name() === allDeviceTypes[deviceId].parentGroup) {
            group = shapeGroup;
        }
    });
    ...
    */
    ...
    if (updateNode) {
        updateNodeAttributes(updateNode);
        return;
    }
    ...
    function updateNodeAttributes(node) {
        ...
        if ('data_vHeight' in attrs && attrs.data_vHeight != '') {
            node.data_vHeight = data_vHeight;
        }

        if ('data_tilt' in attrs && attrs.data_tilt != '') {
            node.data_tilt = data_tilt;
        }

        if ('data_slant' in attrs && attrs.data_slant != '') {
            node.data_slant = data_slant;
        }
        ...
        if (attrs.data_fovHidden) {
            node.data_fovHidden = attrs.data_fovHidden;
        }

        if (attrs.data_audioHidden) {
            node.data_audioHidden = attrs.data_audioHidden;
        }

        if (attrs.data_dispDistHidden) {
            node.data_dispDistHidden = attrs.data_dispDistHidden;
        }

        if ('data_role' in attrs) {
            node.data_role = attrs.data_role; /* data_role.value & data_role.index */
        }

        if ('data_color' in attrs) {
            node.data_color = attrs.data_color; /* data_color.value & data_color.index */
        }
    ...
    if ('speakerRadius' in insertDevice && 'speakerDeg' in insertDevice) {
        ...
        /* audioShadingLine & audioShadingShade combine to form the coverage */
        let speakerShadingShade = new Konva.Arc({
            /* x and y should be tracked in the group only */
            innerRadius: innerRadius,
...
function updateFormatDetails(eventOrShapeId, updateAutoZvalue = false) {
    ...
    if (updateAutoZvalue && document.getElementById('autoZposition').checked === true) {
...
function onKeyDown(e) {
    ...
    trNodesCopy.forEach(shape => {
        ...
        if (tr.nodes().length === 1 || trNodesCopy.length === 1) {
...
function rotateRoom(rotationAmount) {
    ...
    document.getElementById('dialogRotateRoom').showModal();

    if (rotationAmount < 0) {
        document.getElementById('iconDialogRotateRoom').style.transform = 'scaleX(1)';
