const version = "v0.1.632";  /* format example "v0.1" or "v0.2.3" - ver 0.1.1 and 0.1.2 should be compatible with a Shareable Link because ver, v0.0, 0.1 and ver 0.2 are not compatible. */
...
function updateFormatDetails(eventOrShapeId) {
    ...
    roomObj.items[parentGroup].forEach((item, index) => {
        if (item.id === id) {
            ...
            if ((shape.data_deviceid.startsWith('wall') || shape.data_deviceid.startsWith('column') || shape.data_deviced === 'cylinder') && !shape.data_deviceid.startsWith('wallChairs')) {
