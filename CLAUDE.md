...
| Every device class, not just structure | Walls, doors, boxes AND chairs, tables, cameras, displays. An earlier cut drew neighbouring structure only, on the grounds that a neighbour's coverage wedge should not bleed in; the vanishing act above outweighed it, and the coverage is taken off the greyed item instead (next row) |
...
| A canvas-only item is greyed | `applyRoomGhostOpacity()` at `ROOM_PART_GHOST_OPACITY` (0.35). The grey means "this room will not export it", not "you may not touch it". There is no toggle: it is the only thing on screen that distinguishes the two |
| Its coverage comes OFF, not down | Camera, mic, speaker and display coverage (`ROOM_PART_COVERAGE_PREFIXES`) is **hidden** for a greyed item rather than dimmed with it: a wedge cast by the room next door is the one thing the zoomed view is there to keep out, and a faint one still reads as this room's. `setRoomPartCoverageVisible()` does both halves, and coming back on goes through `applyLayerStateToCoverageNodes()` so the per-item `data_*Hidden` flags and the VRC layer decide it. The label rides the dim instead, since it only names the item. Confirmed live: a Room Bar dragged out of the room loses its FOV and greys, dragged back in gets both back, and one carrying `data_fovHidden` stays off through the whole round trip |
| Two levels of coverage visibility | This sets the coverage NODE's own `visible`; the overlay buttons hide the whole `cameraCoverage` / `microphoneCoverage` / … `Konva.Group`. They compose, so nothing here can override a global toggle |
...
- **Edit mode**: everything else below (drag, select, insert, convert,
  ...
  blue (`#89CFF0`)**; the orange bezier control handles also enlarge
  on hover (to ~the anchor hover size, color unchanged) so both kinds
  of point read as clickable. The preview
